...
pub fn initialize_stats_cache(state: &mut AppState) {
    ...
    if IS_STATS_CACHE_INIT.load(Ordering::SeqCst) {
        ...
        state.stats_cache_addr = Some(static_addr_opt.as_ref().unwrap().clone());
    } else {
        let config = state.config.clone();
        INIT_STATS_CACHE.call_once(|| {
            let mut static_addr_opt = STATS_CACHE_MUTEX.lock().unwrap();
            let addr = StatsCache::new(config).start();
            let addr_clone = addr.clone();
            let addr_clone_2 = addr.clone();
            static_addr_opt.get_or_insert(addr);

            state.stats_cache_addr = Some(addr_clone);
            IS_STATS_CACHE_INIT.store(true, Ordering::SeqCst);

            // set cache reset interval
            if state.config.cache_reset_interval_seconds > 0 {
                let mut scheduler = Scheduler::new();
                scheduler
                    .every(state.config.cache_reset_interval_seconds.seconds())
                    .run(move || {
                        let reset_future =
                            addr_clone_2
