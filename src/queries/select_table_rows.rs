...
// use super::query_types::{QueryParamsSelect, QueryResult};
...
// pub fn select_table_rows(client: Client, params: QueryParamsSelect) -> impl Future<Item = (Vec<RowFields>, Client), Error = (ApiError, Client)> {
//     if let Err(e) = validate_sql_name(&params.table) {
//         return Either::A(err::<(Vec<RowFields>, Client), (ApiError, Client)>((e, client)));
//     }
...
//     let fk_columns = ForeignKeyReference::from_query_columns(client, &params.table, &columns)?;
...
//     let prep_statement = client.prepare(&statement)?;
